...
function init() {
  ...
  App.initAlbum()
  console.log(getParseDate('25/02/2021'))
}
...
const App = {
  ...
  async initAlbum() {
    try {
      ...
      if (!album.activo)
        throw Error('El álbum solicitado está inactivo, por favor contáctese al teléfono +54 381 5900868')
  ...
  fillInfoPanel() {
    const { evento, fechaEvento: fecha, lugar, cantidadFotos: fotos, cfgSliderTimer: timer } = this.album
    const { nombre: autor, web, tel, mail: email } = this.album.autor
    const parseDate = parseDates(fecha)
    const imageAndTime = `${fotos} imágenes, ${((fotos * timer) / (timer * 10)).toFixed(1)} minutos.`
...
// Return date parse in format: martes, 10 de enero de 2017
